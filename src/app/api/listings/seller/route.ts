...
export async function GET(req: Request) {
  try {
    ...
    return NextResponse.json((listings ?? []) as ListingWithPhotos[], { headers: AUTH_CACHE_HEADERS });
  } catch (error) {
    console.error('Error fetching seller listings:', error);
    return NextResponse.json({ error: 'Failed to fetch seller listings' }, { status: 500, headers: AUTH_CACHE_HEADERS });
  }
}
